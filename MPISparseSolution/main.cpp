...
int main(int argc, char **argv) {
    ...
    if (rank == MPI_MASTER)
    {
        cout << "Running inference for " << NUM_IMAGES << " images with sparse CUDA matrix multiplication on " << NUM_LAYERS << " layers, each size " << NUM_NEURONS << endl;
    ...
    // For sparse matrix multiplication, we also keep track of the rows entries belong to.
    int** weights_rows = new int *[NUM_LAYERS];
    float** weights_vals = new float *[NUM_LAYERS];

    for (int i = 0; i < NUM_LAYERS; i++) {
        weights_rows[i] = new int[NUM_NEURONS * NUM_NEURONS];
        weights_vals[i] = new float[NUM_NEURONS * NUM_NEURONS];
    ...
    for (int layer_idx = 1; layer_idx <= NUM_LAYERS; layer_idx++) {
        ...
        // We assume that weight layer file is sorted by column. 
        int currCol = -1;
        int numInCol = 0;
        ...
        while (getline(ifs, line)) {
            ...
            // Weight matrices are stored by column to make multiplication easier.
            // We also store the row, so that we don't iterate over zero entries.
            weights_rows[layer_idx - 1][(col - 1) * NUM_NEURONS + numInCol] = row - 1;
            weights_vals[layer_idx - 1][(col - 1) * NUM_NEURONS + numInCol] = value;
    ...
    {
        sparseMatMulCuda(images, weights_rows, weights_vals, truth_results, img_idx_start, img_idx_end);
